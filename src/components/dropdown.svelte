...
<div class="" id="search-select-client">
    <input bind:value={searchInput} on:click={handleOpen} on:blur={handleOpen} placeholder="Selecione o cliente" class="py-2.5 px-3 w-full border-2 rounded-lg box-border focus:outline-none focus:border-blue-modernize" type="text" name="clients"> 
    <div class="flex">
        {#each inputValues as client }
            <div class="flex-auto">{client}</div>  
        {/each}
    </div>
    ...
    {#if isOpen}
        <div transition:slide={{ delay: 150, duration: 300, easing: quintOut, axis: 'x' }} class="rounded border-2 py-2.5 px-3">
            {#each clients as client }
                ...
                <div on:click={handleDropdownOption(client.id)}
